...
<head>
  ...
  <title>{{ __('messages.login') }} - SkillUp Kazakhstan</title>
...
<body>
  ...
  <section class="form-section">
    <div class="form-card">
      <h2>{{ __('messages.login_page') }}</h2>
      <p>{{ __('messages.enter_email_password') }}</p>
      ...
      <form method="POST" action="/login">
        ...
        <button type="submit" class="form-btn">{{ __('messages.login') }}</button>
      </form>
      ...
      <div class="form-link">
        {{ __('messages.no_account') }} <a href="/register">{{ __('messages.register') }}</a>
      </div>
    </div>
  </section>
